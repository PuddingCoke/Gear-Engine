...
![CrytekSponza](Illustrations/cryteksponza.jpg)
...
![FFTOcean](Illustrations/fftocean.jpg)

### DigitalRain
经典《黑客帝国》风格数字雨效果。Point List 配合 Geometry Shader 将每个字符点扩展为始终面向相机的四边形，Pixel Shader 采样字体图集并混合顶点颜色。每列字符独立下落，头部微泛黄亮色、拖尾通过幂函数衰减。TextBatch 系统单次 draw call 批量渲染最多 65536 个字符。含 Bloom 后处理，默认为动态壁纸模式，后台线程检测桌面遮挡时自动暂停渲染。

https://github.com/user-attachments/assets/6d9ab510-d2e1-4f30-9dd6-baf27b2faee6
...
![FluidSimulation](Illustrations/fluidsimulation.jpg)
...
![SimpleParticleEffect](Illustrations/simpleparticleeffect.jpg)

### BlackHole
全屏 Pixel Shader 实现的黑洞引力透镜效果，通过叠加多层噪声和星盘纹理，以指数函数模拟光线在黑洞极强引力场中的弯曲路径，将背景扭曲为环形结构。ImGui 面板可调纹理周期、噪声层级参数等。含 Bloom 后处理。着色器代码来自[sonicether](https://www.shadertoy.com/view/lstSRS)，我改装成了循环模式，可以渲染成动画作为动态壁纸。预渲染好的动态壁纸已上传至 Wallpaper Engine 创意工坊：[点我跳转](https://steamcommunity.com/sharedfiles/filedetails/?id=3583163695)（不过个人感觉调色没调好）。

![BlackHole](Illustrations/blackhole.jpg)

### MandelBrotSet
Compute Shader 实时渲染 Mandelbrot 集分形。鼠标拖拽平移、滚轮缩放，使用帧累积插值实现平滑过渡。ImGui 面板可调缩放和插值参数，含 Bloom 后处理。

![MandelBrotSet](Illustrations/mandelbrotset.jpg)
...
![MandelBulb](Illustrations/mandelbulb.jpg)
...
![SmoothLife](Illustrations/smoothlife.jpg)
...
![SteeringBehavior](Illustrations/steeringbehavior.jpg)

### EncodeTest
简单的视频编码测试程序，用于验证引擎的 NVENC 硬件编码功能。关于离线渲染，我的引擎代码中提供了相关的实现细节，此外我还写了一篇文章：[使用NVENC API编码D3D12纹理](https://www.cnblogs.com/TiredInkRaven/p/18656474)。这里稍微多提一下，离线渲染这个功能是我大二的时候心血来潮在我写的OpenGL老项目中实现的，当时还不能直接搞，必须得利用CUDA这个东西，相关的代码在[这个仓库](https://github.com/PuddingCoke/Cuda-OpenGL-Encode)。后来学D3D11的时候在D3D11老项目中也实现了离线渲染功能，D3D11老项目代码写得太烂了，就不提供相关的实现了。当时真的就是觉得实现这个功能很有意思，觉得可以做点小动画什么的。所以就去网上学了一点和视频编码相关的知识，然后阅读英伟达发布的文档和源代码实现了相关功能。怎么说呢，用实时渲染的API来搞离线渲染可能稍微有点背道而驰，但是我就是觉得有意思，所以实现了这个功能。
