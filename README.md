...
刚开始接触 D3D11 时感觉还好，感觉比 OpenGL 友好一些。但是越用就越不喜欢，尤其是当我学到延迟上下文的时候。使用立即上下文，你可以在每次draw call或dispatch call为着色器提供一些新鲜数据，但是延迟上下文就不行。我非常讨厌这一点，所以当时就搁置 D3D11 老项目学习 D3D12 去了。看了些微软发布的教学视频，我了解到了 D3D12 中的 RootSignature ，每一个 draw call、每一个 dispatch call 你都能通过 Root Signature 利用根常量为着色器提供新鲜数据，但是资源屏障、资源管理、管线状态、命令录制等全部暴露在开发者手中，导致一个三角形就要写几百行代码。我于是开始构思 Gear：**尽可能实现 D3D11 的便捷性，同时尽可能利用 D3D12 的灵活性**。在无数个空闲的碎片时间里，这个引擎就应运而生了。
...
**Bindless 资源绑定** — Gear 的资源绑定框架基于 Shader Model 6.6 的 `ResourceDescriptorHeap` 构建，设计灵感来自[微软 SM 6.6 文档](https://microsoft.github.io/DirectX-Specs/d3d/HLSL_ShaderModel6_6.html)和 [WickedEngine](https://wickedengine.net/2021/04/06/bindless-descriptors/) 的实践。在 Bindless 模型下，所有着色器资源驻留在一个全局描述符堆中，着色器通过索引自由访问任意资源。用户可以像下方的代码那样轻松地读取或修改资源。
...
Bindless 让着色器侧的编程体验极为自然，也让引擎侧的资源绑定框架得以简化——不再需要为每种着色器阶段手动管理烦人的描述符表。

**资源绑定与资源屏障插入自动化** — 用户调用 `setVSConstants`、`setPSConstants` 等方法时传入的是 `ShaderResourceDesc` 等描述结构体（定义于 [PipelineResourceDesc.h](Engine/Inc/Gear/Core/Resource/D3D12Resource/PipelineResourceDesc.h)）。这些结构体不仅记录了资源在描述符堆中的索引，还携带了底层 D3D12 资源的指针和子资源范围信息（mip slice 等），它们由高级资源类（如 `TextureRenderView`）通过 `getAllSRVIndex()`、`getRTVMipHandle()` 等方法对外提供。`GraphicsContext` 根据结构体中的信息自动完成三件事：解析目标资源状态 → 追踪并插入资源转换屏障 → 绑定至对应的着色器阶段。draw call 和 dispatch call 前资源屏障自动 flush，用户无需手动管理任何一个 `D3D12_RESOURCE_BARRIER`。
...
预编译的 .lib 和 .dll 以及其它的依赖资源已放置在 `Libs/` 和 `Dependencies/` 目录中。SampleProject项目已被恰当配置，它的链接器的输入的附加依赖项中已经添加了所有需要链接的静态库。此外，编译后会自动拷贝 `Dependencies/` 目录下的所有文件到EXE输出目录。
