...
namespace Gear::Core::D3D12Core
{
	...
	DescriptorHandle::DescriptorHandle(const CD3DX12_CPU_DESCRIPTOR_HANDLE cpuHandleStart, const CD3DX12_GPU_DESCRIPTOR_HANDLE gpuHandleStart,
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE DescriptorHandle::getCurrentCPUHandle() const
	{
		return CD3DX12_CPU_DESCRIPTOR_HANDLE(cpuHandleStart, getCurrentIndex(), incrementSize);
	}

	CD3DX12_GPU_DESCRIPTOR_HANDLE DescriptorHandle::getCurrentGPUHandle() const
	{
	...
	uint32_t DescriptorHandle::getIncrementSize() const
	{
		return incrementSize;
	}

	uint32_t DescriptorHandle::getDistToEnd() const
	{
		return offsetLimit - currentOffset;
	}

	const DescriptorHeap* DescriptorHandle::getDescriptorHeap() const
	{
		return descriptorHeap;
	}

	void DescriptorHandle::move(const uint32_t num)
	{
		currentOffset += num;
	...
	void DescriptorHandle::copyDescriptors(const uint32_t numCopy, DescriptorHandle copyDestHandle, DescriptorHandle copySrcHandle, const D3D12_DESCRIPTOR_HEAP_TYPE heapType)
	{
	...
	void DescriptorHandle::copyDescriptorsSimple(const uint32_t numCopy, DescriptorHandle& copyDestHandle, DescriptorHandle& copySrcHandle, const D3D12_DESCRIPTOR_HEAP_TYPE heapType)
	{
