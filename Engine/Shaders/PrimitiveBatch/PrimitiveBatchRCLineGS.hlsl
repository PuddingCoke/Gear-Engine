...
)
{
    ...
    output.pos = mul(input[0].pos - float4(dir, 0.0, 0.0), perframeResource.proj);
    ...
    for (uint i = 0; i < EDGENUM / 2; i++)
    {
        ...
        output.pos = mul(input[0].pos + float4(v0, 0.0, 0.0), perframeResource.proj);
        outputs.Append(output);
        
        output.pos = mul(input[0].pos + float4(v1, 0.0, 0.0), perframeResource.proj);
        outputs.Append(output);
    }
    
    output.pos = mul(input[1].pos + float4(v0, 0.0, 0.0), perframeResource.proj);
    outputs.Append(output);
        
    output.pos = mul(input[1].pos + float4(v1, 0.0, 0.0), perframeResource.proj);
    outputs.Append(output);
    ...
    output.pos = mul(input[1].pos + float4(dir, 0.0, 0.0), perframeResource.proj);
    outputs.Append(output);
}